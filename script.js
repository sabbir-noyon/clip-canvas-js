  ...
  data.data.forEach((mainData) => {
    ...
    divForCards.innerHTML = `
      <div class="cards pb-6">
        ...
        ${
          mainData?.others?.posted_date !== ""
            ? `<p class="p-2 bg-gray-950 w-max text-white text-sm font-medium rounded-lg relative bottom-10 lg:left-60 md:left-72 left-2/4">
                ${convertToHoursMinutes(mainData?.others?.posted_date)}
              </p>`
            : ""
        }
        ...
        <div class="alla-body flex">
          <div class="profile-img h-10 rounded min-w-fit pl-2 mt-5 mr-4">
            ${
              mainData?.others?.posted_date !== ""
                ? `<img class="w-10 h-10 -mt-8 ml-2 rounded-full" src="${mainData?.authors[0].profile_picture}">`
                : `<img class="w-10 h-10 mt-2 ml-2 rounded-full" src="${mainData?.authors[0].profile_picture}">`
            }
          </div>
          ...
          <div class="main">
            ${
              mainData?.others?.posted_date !== ""
                ? `<h4 class="-mt-6 text-2xl font-bold text-[#171717]">${mainData?.title}</h4>`
            ...
            <div class="flex">
              ...
              ${
                mainData?.authors[0].verified !== false &&
                mainData?.authors[0].verified !== ""
                  ? '<img class="w-8 h-8 mt-2 ml-2" src="icons/Twitter_Verified_Badge.svg.png">'
                  : ""
              }
            </div>
...
const allDataByClick = async (kolamula) => {
  const response = await fetch(
    `https://openapi.programming-hero.com/api/videos/category/${kolamula}`
  );
  const data = await response.json();
  ...
  if (data.data.length === 0) {
    ...
    noDataDiv.innerHTML = `
      <div class="flex flex-col items-center justify-center min-h-[60vh]">
        <img class="mx-auto text-center" src="icons/Icon.png" />
        <p class="text-2xl lg:text-4xl font-bold mt-10 text-center">
